...
def back():
    ...
    exloop_calc_frame.pack_forget()
    exloop_calc_result.pack_forget()

    supp_backbutton.pack_forget()
    rc_backbutton.pack_forget()
    exloop_backbutton.pack_forget()

    supp_desc_frame.pack_forget()
    rc_desc_frame.pack_forget()
    exloop_desc_frame.pack_forget()
    ...
    rc_soilreaction_click.set(" ")
    rc_resilientsoil_click.set(" ")
    hydropress_click.set(" ")
...
def calculator_start(): #Function for opening calculator module
    ...
    if sel_calc == "Onshore Expansion Loop Calculator":
        ...
        rc_calc_def.pack_forget()
        exloop_calc_def.pack_forget()
        exloop_backbutton.pack(side=BOTTOM)
        exloop_calc_frame.pack(side=TOP, padx=10, pady=10)
...
def calc_opt_sel(sel_calc): #Function for module description preview
    ...
    if sel_calc == "Onshore Expansion Loop Calculator":
        ...
        supp_calc_def.pack_forget()
        rc_calc_def.pack_forget()
        exloop_desc_frame.pack(padx=10, pady=10)
        exloop_calc_def.pack()
...
def code_sel(sel_code):
    ...
    if code=="ASME B31.4":
        ...
        des_fac = float(0.72)
    if code=="ASME B31.8":
        if str(calc_opt_click.get()) == "Onshore Pipe Support Spacing Calculator":
            lc_label.grid(row=1, column=0, pady=2)
            lc_drop.grid(row=1, column=1, columnspan=2, pady=2)

        if str(calc_opt_click.get()) == "Onshore Pipeline Road Crossing Calculator":
            rc_lc_label.grid(row=1, column=0, pady=2)
            rc_lc_drop.grid(row=1, column=1, columnspan=2, pady=2)

        if str(calc_opt_click.get()) == "Onshore Expansion Loop Calculator":
            exloop_lc_label.grid(row=1, column=0, pady=2)
            exloop_lc_drop.grid(row=1, column=1, columnspan=2, pady=2)
...
def contactarea_sel(sel_contactarea):
    ...
    if sel_contactarea == "User Defined":
        rc_contactareainput_label.grid(row=17, column=0, pady=2)
        rc_contactarea_entry.grid(row=17, column=1, columnspan=2, pady=2)
...
def axleload_sel(sel_axleload):
    ...
    if sel_axleload == "User Defined":
        rc_axleloadinput_label.grid(row=19, column=0, pady=2)
        rc_axleload_entry.grid(row=19, column=1, columnspan=2, pady=2)
    if sel_axleload == "API Standard":
        rc_axleload_entry.grid_remove()
        rc_axleloadinput_label.grid_remove()

#Expansion Loop Function Definition
def hydropress_sel(sel_hyd):
    sel_hyd = str(hydropress_click.get())
    global exloop_phydro
    global exloop_designpressure
    exloop_designpressure = float(exloop_designpressure_entry.get()) * float(6894.76)
    if sel_hyd == "1.25 x Design Pressure":
        exloop_phydro = 1.25 * (exloop_designpressure)
    if sel_hyd == "1.5 x Design Pressure":
        exloop_phydro = 1.5 * (exloop_designpressure)
    if sel_hyd == "User Defined":
        exloop_hydroratio_label = customtkinter.CTkLabel(exloop_calc_frame, text="Hydrotest Pressure Ratio: ").grid(row=10, column=0, pady=2)
        exloop_hydroratio_entry = customtkinter.CTkEntry(exloop_calc_frame, justify= CENTER, width=140)
        exloop_hydroratio_entry.grid(row=10, column=1, columnspan=2, pady=2)
        exloop_phydro = float(exloop_hydroratio_entry.get()) * (exloop_designpressure)
...
def calc():
    if str(calc_opt_click.get()) == "Onshore Pipe Support Spacing Calculator":
        ...
        def_result_empty.grid(row=20, column=1)
        def_result_filled.grid(row=20, column=2)

        sls_result_empty.grid(row=21, column=1)
        sls_result_filled.grid(row=21, column=2)

        uls_result_empty.grid(row=22, column=1)
        uls_result_filled.grid(row=22, column=2)


        pipe_condition_label = customtkinter.CTkLabel(supp_calc_result, text="Pipe Condition").grid(row=18, column=1, columnspan=2)
        max_span_label = customtkinter.CTkLabel(supp_calc_result, text="Maximum Support Spacing").grid(row=18, column=0, rowspan=2)
        empty_label = customtkinter.CTkLabel(supp_calc_result, text="Empty").grid(row=19, column=1)
        filled_label = customtkinter.CTkLabel(supp_calc_result, text="Filled with Water").grid(row=19, column=2)

        result_def_label = customtkinter.CTkLabel(supp_calc_result, text="Deflection Limited: ").grid(row=20, column=0)
        result_sls_label = customtkinter.CTkLabel(supp_calc_result, text="Serviceability Limit State: ").grid(row=21, column=0)
        result_uls_label = customtkinter.CTkLabel(supp_calc_result, text="Ultimate Limit State: ").grid(row=22, column=0)
    if str(calc_opt_click.get()) == "Onshore Pipeline Road Crossing Calculator":
        root.geometry("1200x700")
        rc_calc_result.pack(padx=10, pady=10, side=RIGHT, anchor="n")

        rc_nps = int(nps_click.get())
        rc_od = od
        rc_smys = smys
        ...
        thicknessexcoat=float(rc_excoat_thk_entry.get())
        ODexcoat = rc_od + (2 * float(thicknessexcoat) / 1000)

        if rc_installmethod_click.get() == "Auger Bored":
            boreddiameter = ODexcoat + float(0.58)
        if rc_installmethod_click.get() == "Open Cut":
            boreddiameter = ODexcoat

        barlowint_= ((maop * (rc_od - pipethickness)) / (2 * pipethickness)) / 1000000
        barlowint=round(barlowint_,3)

        allowablestress_=(smys*des_fac) / 1000000
    ...
    #circumferential stress due to earth load
        ...
        modulussoilreaction_input=float(rc_soilreaction_click.get())
    ...
    #Cyclic Circumferential Stress
        ...
        resilientmodulus_input=float(rc_resilientsoil_click.get())
    ...
    #Cyclic Longitudinal Stress
        ...
        girthlimit_=((12000*6894.76) *des_fac) /1000000
        girthlimit=round(girthlimit_,3)
        longlimit_=((23000*6894.76) *des_fac) /1000000
        ...
        rc_allowablestress_result=customtkinter.CTkLabel(rc_calc_result, text=str(allowablestress)+ " MPa").grid(row=0, column=1, columnspan=2)
        rc_barlowstress_result=customtkinter.CTkLabel(rc_calc_result, text=str(barlowstress)+ " MPa").grid(row=1, column=1, columnspan=2)
        rc_barlowunitycheck_result.grid(row=2, column=1, columnspan=2)
        rc_circuminternalstress_result = customtkinter.CTkLabel(rc_calc_result, text=str(barlowint)+ " MPa").grid(row=3, column=1, columnspan=2)
        rc_circumstress_result = customtkinter.CTkLabel(rc_calc_result,text=str(circumstressearthload)+ " MPa").grid(row=4,column=1,columnspan=2)
        rc_surfaceload_result = customtkinter.CTkLabel(rc_calc_result,text=str(round((applieddesignsurfacepressure_/1000000),3))+ " MPa").grid(row=5,column=1,columnspan=2)
        rc_impactfactor_result = customtkinter.CTkLabel(rc_calc_result,text=str(impactfactor)).grid(row=6,column=1,columnspan=2)
        rc_cycliccircumstress_result = customtkinter.CTkLabel(rc_calc_result,text=str(cycliccircumferentialstress)+ " MPa").grid(row=7,column=1,columnspan=2)
        rc_cycliclongstress_result = customtkinter.CTkLabel(rc_calc_result,text=str(cycliclongitudinalstress)+ " MPa").grid(row=8,column=1,columnspan=2)
        rc_maxcircumstress_result = customtkinter.CTkLabel(rc_calc_result, text=str(maxcircumferentialstress)+ " MPa").grid(row=9,column=1,columnspan=2)
        rc_maxlongstress_result = customtkinter.CTkLabel(rc_calc_result,text=str(maxlongitudinalstress)+ " MPa").grid(row=10,column=1,columnspan=2)
        rc_maxradialstress_result = customtkinter.CTkLabel(rc_calc_result,text=str(maxradialstress)+ " MPa").grid(row=11,column=1,columnspan=2)
        rc_effectivestress_result = customtkinter.CTkLabel(rc_calc_result,text=str(effectivestress)+ " MPa").grid(row=12,column=1,columnspan=2)
        rc_effectivestress_unitycheck.grid(row=13,column=1,columnspan=2)
        rc_girthallow.grid(row=14,column=1,columnspan=2)
        rc_girthunitycheck.grid(row=15,column=1,columnspan=2)
        rc_longallow.grid(row=16,column=1,columnspan=2)
        rc_longunitycheck.grid(row=17,column=1,columnspan=2)
        ...
        if barlowunitycheck == effectivestress_unitycheck == girthunitycheck == longunitycheck == "SAFE":
            ...
            rc_summary_label = customtkinter.CTkLabel(rc_calc_result, text=rc_summary, fg_color="#AAFF00")
        else:
            rc_summary = "Need Pipe Casing"
            rc_summary_label = customtkinter.CTkLabel(rc_calc_result, text=rc_summary, width=196, fg_color="#F33A6A")

        rc_summary_label.pack_forget()
        rc_summary_label.grid(row=18,column=1,columnspan=2)

    if str(calc_opt_click.get()) == "Onshore Expansion Loop Calculator":
        root.geometry("800x700")
        exloop_calc_result.pack(padx=10, pady=10)
        vonmisses_allow = 0.9 * smys
        longitudinal_allow = des_fac * smys

        exloop_pipethickness = float(exloop_thickness_entry.get()) / 1000
        idpipe = od - (2 * exloop_pipethickness)
        pipesectionalarea = 0.25 * math.pi * ((od ** 2) - (idpipe ** 2))

        steel_modulus = float(207000000000)
        thermalexpansioncoeff = float(0.0000117)
        thermalexansionforce = pipesectionalarea * thermalexpansioncoeff * steel_modulus * ((float(exloop_designtemperature_entry.get())) - (float(exloop_ambienttemperature_entry.get())))
        pressureexpansionforce_design = (1 - (2 * 0.3)) * 0.25 * math.pi * (idpipe ** 2) * exloop_designpressure
        pressureexpansionforce_hydro = (1 - (2 * 0.3)) * 0.25 * math.pi * (idpipe ** 2) * exloop_phydro
        totalaxialforce_design = thermalexansionforce + pressureexpansionforce_design
        totalaxialforce_hydro = thermalexansionforce + pressureexpansionforce_hydro
        effectivespanlength = float(exloop_spanlength_entry.get())

        totalexpansionlength_design = ((totalaxialforce_design * effectivespanlength) / (steel_modulus * pipesectionalarea))
        totalexpansionlength_hydro = ((totalaxialforce_hydro * effectivespanlength) / (steel_modulus * pipesectionalarea))

        exloop_hoopstress = (exloop_designpressure * od) / (2 * exloop_pipethickness)
        exloop_longstress_restrained = (0.3 * exloop_hoopstress) + (steel_modulus * thermalexpansioncoeff * (float(exloop_designtemperature_entry.get()) - float(exloop_ambienttemperature_entry.get())))
        exloop_longstress_unrestrained = (exloop_designpressure * od) / (4 * exloop_pipethickness)
        exloop_longallow_a = (2 * exloop_hoopstress + (((-2 * (exloop_hoopstress ** 2)) + (4 * (vonmisses_allow ** 2 - exloop_hoopstress ** 2))) ** 0.5)) / 2
        exloop_longallow_b = (2 * exloop_hoopstress - (((-2 * (exloop_hoopstress ** 2)) + (4 * (vonmisses_allow ** 2 - exloop_hoopstress ** 2))) ** 0.5)) / 2

        if exloop_longallow_a > 0 and exloop_longallow_b > 0:
            equivalentallowable = min(longitudinal_allow, exloop_longallow_b, exloop_longallow_a)
        else:
            equivalentallowable = longitudinal_allow

        exloop_bendingstress_1 = equivalentallowable - exloop_longstress_unrestrained
        exloop_bendingstress_2 = longitudinal_allow - exloop_longstress_unrestrained
        minimumbendingstress = min(exloop_bendingstress_1, exloop_bendingstress_2)

        looparmequation_hydro_1 = 3 * steel_modulus * od * (totalexpansionlength_hydro / 2) / (2 * minimumbendingstress)
        looparmequation_design_1 = 3 * steel_modulus * od * (totalexpansionlength_design / 2) / (2 * minimumbendingstress)
        looparmequation_2 = ((exloop_pipethickness * 5 * od) / (0.5 * (od - exloop_pipethickness) ** 2)) ** (2 / 3)
        minimumarmlength_hydro = round((0.9 / looparmequation_2 * looparmequation_hydro_1) ** 0.5, 3)
        minimumarmlength_design = round((0.9 / looparmequation_2 * looparmequation_design_1) ** 0.5, 3)
        reccommended_arm_length = math.ceil(minimumarmlength_hydro)

        exloop_total_expansion_design = customtkinter.CTkLabel(exloop_calc_result, text="Total Expansion (Design): ").grid(row=0, column=0)
        exloop_total_expansion_hydro = customtkinter.CTkLabel(exloop_calc_result,text="Total Expansion (Hydrotest): ").grid(row=1, column=0)
        exloop_minarm_design = customtkinter.CTkLabel(exloop_calc_result, text="Minimum Required Arm Length (Design): ").grid(row=2, column=0)
        exloop_minarm_hydro = customtkinter.CTkLabel(exloop_calc_result,text="Minimum Required Arm Length (Hydrotest): ").grid(row=3,column=0)
        exloop_recommended_armlength = customtkinter.CTkLabel(exloop_calc_result, text="Minimum Selected Arm Length (m): ").grid(row=4,  column=0)

        explength_design = round(totalexpansionlength_design*1000, 3)
        explength_hydro = round(totalexpansionlength_hydro*1000, 3)
        total_expansion_design = customtkinter.CTkLabel(exloop_calc_result, text=str(explength_design) + " mm").grid(row=0,column=1)
        total_expansion_hydro = customtkinter.CTkLabel(exloop_calc_result,text=str(explength_hydro) + " mm").grid(row=1,column=1)
        minarm_design = customtkinter.CTkLabel(exloop_calc_result, text=str(minimumarmlength_design) + " m").grid(row=2,column=1)
        minarm_hydro = customtkinter.CTkLabel(exloop_calc_result, text=str(minimumarmlength_hydro) + " m").grid(row=3, column=1)
        recomm_arm = customtkinter.CTkLabel(exloop_calc_result, text=str(reccommended_arm_length) + " m").grid(row=4, column=1)
...
exloop_calc_def=LabelFrame(root, text="Module Description", padx=5, pady=5)
...
#Expansion Loop - LabelFrame Definitions
exloop_calc_frame=LabelFrame(root, text="Expansion Loop Calculator", padx=10, pady=10)
exloop_calc_result=LabelFrame(root, text="Calculation Results", padx=10, pady=10)
...
#Expansion Loop Frame Definition
exloop_desc_frame=Frame(exloop_calc_def, height=800, width=800)
...
#Expansion Loop Calculator Image Definition
desc_exloop = ImageTk.PhotoImage(Image.open("exloopdesc.png"))
exloop_desc_label = Label(exloop_desc_frame, image=desc_exloop, padx=10, pady=10)
exloop_desc_label.pack()

#Pipe Support Spacing Label Definition
code_label=customtkinter.CTkLabel(supp_calc_frame, text="Standard Code: ", padx=10).grid(row=0, column=0)

lc_label=customtkinter.CTkLabel(supp_calc_frame, text="Location Class: ")

class_label=customtkinter.CTkLabel(supp_calc_frame, text="Pipe Material Class: ").grid(row=3, column=0)

nps_label=customtkinter.CTkLabel(supp_calc_frame, text="Pipe NPS (in): ").grid(row=4, column=0)

thk_label=customtkinter.CTkLabel(supp_calc_frame, text="Pipe Thickness (mm): ").grid(row=5, column=0)

excoat_exist_label=customtkinter.CTkLabel(supp_calc_frame, text="Any External Coating Existence?").grid(row=6, column=0)
...
insul_exist_label=customtkinter.CTkLabel(supp_calc_frame, text="Any Insulation Existence?").grid(row=9, column=0)
...
incoat_exist_label=customtkinter.CTkLabel(supp_calc_frame, text="Any Internal Coating Existence?").grid(row=12, column=0)
...
deflection_label=customtkinter.CTkLabel(supp_calc_frame, text="Permissible Deflection at Midspan (mm): ").grid(row=15, column=0)

#Road Crossing Calculator Label Definition
rc_code_label=customtkinter.CTkLabel(rc_calc_frame, text="Standard Code: ").grid(row=0, column=0)

rc_lc_label=customtkinter.CTkLabel(rc_calc_frame, text="Location Class: ")

rc_class_label=customtkinter.CTkLabel(rc_calc_frame, text="Pipe Material Class: ").grid(row=3, column=0)

rc_nps_label=customtkinter.CTkLabel(rc_calc_frame, text="Pipe NPS (in): ").grid(row=4, column=0)

rc_thk_label=customtkinter.CTkLabel(rc_calc_frame, text="Pipe Thickness (mm): ").grid(row=5, column=0)

rc_excoat_thk_label=customtkinter.CTkLabel(rc_calc_frame, text="External Coating Thickness (mm): ").grid(row=6, column=0)

rc_maop_label=customtkinter.CTkLabel(rc_calc_frame, text="Max. Operating Pressure (psi): ").grid(row=7, column=0)
rc_maopinfo_label=customtkinter.CTkLabel(rc_calc_frame, text="*For Casing Pipe, Give Zero Value").grid(row=7, column=3)

rc_maot_label=customtkinter.CTkLabel(rc_calc_frame, text="Max. Operating Temperature (deg.C): ").grid(row=8, column=0)
rc_maotinfo_label=customtkinter.CTkLabel(rc_calc_frame, text="*For Casing Pipe, Input Ambient Temperature").grid(row=8, column=3)

rc_ambtemp_label=customtkinter.CTkLabel(rc_calc_frame, text="Minimum Ambient Temperature (deg.C): ").grid(row=9, column=0)

rc_installmethod_label=customtkinter.CTkLabel(rc_calc_frame, text= "Installation Method: ").grid(row=10, column=0)

rc_soiltype_label=customtkinter.CTkLabel(rc_calc_frame, text="Soil Type: ").grid(row=11, column=0)

rc_soilunit_label=customtkinter.CTkLabel(rc_calc_frame, text="Unit Weight of Soil (kN/m3): ").grid(row=12, column=0)

rc_soilreaction_label=customtkinter.CTkLabel(rc_calc_frame, text="Modulus of Soil Reaction (ksi): ").grid(row=13, column=0)
rc_soilreactioninfo_label=customtkinter.CTkLabel(rc_calc_frame, text="*Refer to Annex A.1 Table A-1 of API 1102").grid(row=13, column=3)

rc_resilientsoil_label=customtkinter.CTkLabel(rc_calc_frame, text="Resilient Modulus of Soil (ksi): ").grid(row=14, column=0)
rc_resilientsoilinfo_label=customtkinter.CTkLabel(rc_calc_frame, text="*Refer to Annex A.1 Table A-2 of API 1102").grid(row=14, column=3)

rc_pavement_label=customtkinter.CTkLabel(rc_calc_frame, text="Pavement Type: ").grid(row=15, column=0)

rc_contactarea_label=customtkinter.CTkLabel(rc_calc_frame, text="Contact Area of Wheel: ").grid(row=16, column=0)
rc_contactareainput_label=customtkinter.CTkLabel(rc_calc_frame, text="Contact Area of Wheel (m2): ")
rc_contactareainfo_label=customtkinter.CTkLabel(rc_calc_frame, text="*See Section 4.7.2.2.1 of API 1102").grid(row=16, column=3)

rc_axleload_label=customtkinter.CTkLabel(rc_calc_frame, text="Axle Load: ").grid(row=18, column=0)
rc_axleloadinput_label=customtkinter.CTkLabel(rc_calc_frame, text="Axle Load (kN): ")
rc_axleloadinfo_label=customtkinter.CTkLabel(rc_calc_frame, text="*See Section 4.7.2.2.1 of API 1102").grid(row=18, column=3)

rc_burial_label=customtkinter.CTkLabel(rc_calc_frame, text="Pipe Burial Depth to Top of Pipe (m): ").grid(row=20, column=0)

rc_allowablestress_label=customtkinter.CTkLabel(rc_calc_result, text="Allowable Stress: ").grid(row=0, column=0)
rc_result_barlowstress_label=customtkinter.CTkLabel(rc_calc_result, text="Barlow Stress: ").grid(row=1, column=0)
rc_result_ucbarlowstress_label=customtkinter.CTkLabel(rc_calc_result, text="Barlow Stress Unity Check: ").grid(row=2, column=0)
rc_circuminternalstress_label=customtkinter.CTkLabel(rc_calc_result, text="Circumferential Internal Pressure: ").grid(row=3, column=0)
rc_circumstress_label=customtkinter.CTkLabel(rc_calc_result, text="Circumferential Stress Due to Earth Load: ").grid(row=4, column=0)
rc_surfaceload_label=customtkinter.CTkLabel(rc_calc_result, text="Applied Surface Load: ").grid(row=5, column=0)
rc_impactfactor_label=customtkinter.CTkLabel(rc_calc_result, text="Impact Factor: ").grid(row=6, column=0)
rc_cycliccircum_stress_label=customtkinter.CTkLabel(rc_calc_result, text="Cyclic Circumferential Stress: ").grid(row=7, column=0)
rc_cycliclong_stress_label=customtkinter.CTkLabel(rc_calc_result, text="Cyclic Longitudinal Stress: ").grid(row=8, column=0)
rc_maxcircumstress_label=customtkinter.CTkLabel(rc_calc_result, text="Max. Circumferential Stress: ").grid(row=9, column=0)
rc_maxlongstress_label=customtkinter.CTkLabel(rc_calc_result, text="Max. Longitudinal Stress: ").grid(row=10, column=0)
rc_maxradialstress_label=customtkinter.CTkLabel(rc_calc_result, text="Max. Radial Stress: ").grid(row=11, column=0)
rc_effectivestress_label=customtkinter.CTkLabel(rc_calc_result, text="Effective Stress: ").grid(row=12, column=0)
rc_effstressunity_label=customtkinter.CTkLabel(rc_calc_result, text="Effective Stress Unity Check: ").grid(row=13, column=0)
rc_girthlimit_label=customtkinter.CTkLabel(rc_calc_result, text="Girth Weld Fatigue Limit: ").grid(row=14, column=0)
rc_girthunity_label=customtkinter.CTkLabel(rc_calc_result, text="Girth Weld Fatigue Check: ").grid(row=15, column=0)
rc_longlimit_label=customtkinter.CTkLabel(rc_calc_result, text="Longitudinal Weld Fatigue Limit: ").grid(row=16, column=0)
rc_longunity_label=customtkinter.CTkLabel(rc_calc_result, text="Longitudinal Weld Fatigue Check: ").grid(row=17, column=0)

rc_assessment_label=customtkinter.CTkLabel(rc_calc_result, text="Assessment Result: ").grid(row=18, column=0)

#Expansion Loop - Label Definitions
exloop_code_label=customtkinter.CTkLabel(exloop_calc_frame, text="Standard Code: ").grid(row=0, column=0)
exloop_lc_label=customtkinter.CTkLabel(exloop_calc_frame, text="Location Class: ")
exloop_class_label=customtkinter.CTkLabel(exloop_calc_frame, text="Pipe Material Class: ").grid(row=3, column=0)
exloop_nps_label=customtkinter.CTkLabel(exloop_calc_frame, text="Pipe NPS (in): ").grid(row=4, column=0)
exloop_thk_label=customtkinter.CTkLabel(exloop_calc_frame, text="Pipe Thickness (mm): ").grid(row=5, column=0)

exloop_designpressure_label=customtkinter.CTkLabel(exloop_calc_frame, text="Design Pressure (psi): ").grid(row=6, column=0)
exloop_designtemperature_label=customtkinter.CTkLabel(exloop_calc_frame, text="Design Temperature (deg.C): ").grid(row=7, column=0)
exloop_ambienttemperature_label=customtkinter.CTkLabel(exloop_calc_frame, text="Min. Ambient Temperature (deg.C): ").grid(row=8, column=0)
exloop_hydrotestpressure_label=customtkinter.CTkLabel(exloop_calc_frame, text="Hydrotest Pressure: ").grid(row=9, column=0)

exloop_spanlength_label=customtkinter.CTkLabel(exloop_calc_frame, text="Effective Span Length (m): ").grid(row=11, column=0)
...
rc_soilreaction_click=customtkinter.StringVar()
rc_resilientsoil_click=customtkinter.StringVar()
rc_installmethod_click=customtkinter.StringVar()
hydropress_click=customtkinter.StringVar()

#Option Definition
calc_opt_opt = ["Onshore Pipe Support Spacing Calculator",
                "Onshore Expansion Loop Calculator",
...
rc_installmethod_op=["Open Cut", "Auger Bored"]
...
rc_soilreaction_op=["0.2", "0.5", "1", "2"]
rc_resilientsoil_op=["5", "10", "20"]

hydropress_op=["1.25 x Design Pressure", "1.5 x Design Pressure", "User Defined"]
...
rc_code_drop=customtkinter.CTkComboBox(rc_calc_frame, variable=code_click, values=code_op, command=code_sel).grid(row=0, column=1, columnspan=2, pady=2)
rc_lc_drop=customtkinter.CTkComboBox(rc_calc_frame, variable=lc_click, values=lc_op, command=lc_sel)
rc_class_drop=customtkinter.CTkComboBox(rc_calc_frame, variable=class_click, values=class_op, command=class_sel).grid(row=3, column=1, columnspan=2, pady=2)
rc_nps_drop=customtkinter.CTkComboBox(rc_calc_frame, variable=nps_click, values=nps_op, command=nps_sel).grid(row=4, column=1, columnspan=2, pady=2)
rc_installmethod_drop=customtkinter.CTkComboBox(rc_calc_frame,  variable=rc_installmethod_click, values=rc_installmethod_op).grid(row=10, column=1, columnspan=2, pady=2)
rc_soiltype_drop=customtkinter.CTkComboBox(rc_calc_frame, variable=rc_soil_click, values=rc_soil_op, command=soil_sel).grid(row=11, column=1, columnspan=2, pady=2)
rc_pavement_drop=customtkinter.CTkComboBox(rc_calc_frame, variable=rc_pavement_click, values=rc_pavement_op, command=pavement_sel).grid(row=15, column=1, columnspan=2, pady=2)
rc_contactarea_drop=customtkinter.CTkComboBox(rc_calc_frame, variable=rc_contactarea_click, values=rc_contactarea_op, command=contactarea_sel).grid(row=16, column=1, columnspan=2, pady=2)
rc_axleload_drop=customtkinter.CTkComboBox(rc_calc_frame, variable=rc_axleload_click, values=rc_axleload_op, command=axleload_sel).grid(row=18, column=1, columnspan=2, pady=2)
rc_soilreaction__drop=customtkinter.CTkComboBox(rc_calc_frame, variable=rc_soilreaction_click, values=rc_soilreaction_op).grid(row=13, column=1, columnspan=2, pady=2)
rc_resilientsoil_drop=customtkinter.CTkComboBox(rc_calc_frame, variable=rc_resilientsoil_click, values=rc_resilientsoil_op).grid(row=14, column=1, columnspan=2, pady=2)

#Expansion Loop Dropdown Definition
exloop_code_drop=customtkinter.CTkComboBox(exloop_calc_frame, variable=code_click, values=code_op, command=code_sel).grid(row=0, column=1, columnspan=2, pady=2)
exloop_lc_drop=customtkinter.CTkComboBox(exloop_calc_frame, variable=lc_click, values=lc_op, command=lc_sel)
exloop_class_drop=customtkinter.CTkComboBox(exloop_calc_frame, variable=class_click, values=class_op, command=class_sel).grid(row=3, column=1, columnspan=2, pady=2)
exloop_nps_drop=customtkinter.CTkComboBox(exloop_calc_frame, variable=nps_click, values=nps_op, command=nps_sel).grid(row=4, column=1, columnspan=2, pady=2)
exloop_hydropress_drop=customtkinter.CTkComboBox(exloop_calc_frame, variable=hydropress_click, values=hydropress_op, command=hydropress_sel).grid(row=9, column=1, columnspan=2, pady=2)
...
rc_soilunit_entry.grid(row=12, column=1, columnspan=2, pady=2)
...
rc_burial_entry.grid(row=20, column=1, columnspan=2, pady=2)

#Expansion Loop Entrybox Definition
exloop_thickness_entry=customtkinter.CTkEntry(exloop_calc_frame, justify= CENTER, width=140)
exloop_thickness_entry.grid(row=5, column=1, columnspan=2, pady=2)

exloop_designpressure_entry=customtkinter.CTkEntry(exloop_calc_frame, justify= CENTER, width=140)
exloop_designpressure_entry.grid(row=6, column=1, columnspan=2, pady=2)

exloop_designtemperature_entry=customtkinter.CTkEntry(exloop_calc_frame, justify= CENTER, width=140)
exloop_designtemperature_entry.grid(row=7, column=1, columnspan=2, pady=2)

exloop_ambienttemperature_entry=customtkinter.CTkEntry(exloop_calc_frame, justify= CENTER, width=140)
exloop_ambienttemperature_entry.grid(row=8, column=1, columnspan=2, pady=2)

exloop_spanlength_entry=customtkinter.CTkEntry(exloop_calc_frame, justify= CENTER, width=140)
exloop_spanlength_entry.grid(row=11, column=1, columnspan=2, pady=2)
...
submit_button=customtkinter.CTkButton(supp_calc_frame, corner_radius=5, text="Submit Data and Calculate!", command=calc).grid(row=16, column=1, columnspan=2, pady=10)
supp_backbutton=customtkinter.CTkButton(root, corner_radius=5, text="Back to Main Menu", fg_color="gray75", command=back)

#Road Crossing Calculator Button Definition
rc_submit_button=customtkinter.CTkButton(rc_calc_frame, corner_radius=5, text="Submit Data and Calculate!", command=calc).grid(row=21, column=2, pady=10)
rc_backbutton=customtkinter.CTkButton(root, corner_radius=5, text="Back to Main Menu", fg_color="gray75", command=back)

#Expansion Loop Button Definition
exloop_submit_button=customtkinter.CTkButton(exloop_calc_frame, corner_radius=5, text="Submit Data and Calculate!", command=calc).grid(row=12, column=2, pady=10)
exloop_backbutton=customtkinter.CTkButton(root, corner_radius=5, text="Back to Main Menu", fg_color="gray75", command=back)
