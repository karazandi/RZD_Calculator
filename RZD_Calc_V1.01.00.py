...
def calc():
    ...
    if str(calc_opt_click.get()) == "Onshore Expansion Loop Calculator":
        ...
        exloop_total_expansion_design = customtkinter.CTkLabel(exloop_calc_result, text="Total Expansion (Design): ").grid(row=0, column=0, pady=2)
        exloop_total_expansion_hydro = customtkinter.CTkLabel(exloop_calc_result,text="Total Expansion (Hydrotest): ").grid(row=1, column=0,pady=2)
        exloop_minarm_design = customtkinter.CTkLabel(exloop_calc_result, text="Minimum Required Arm Length (Design): ").grid(row=2, column=0, pady=2)
        exloop_minarm_hydro = customtkinter.CTkLabel(exloop_calc_result,text="Minimum Required Arm Length (Hydrotest): ").grid(row=3,column=0, pady=2)
        exloop_recommended_armlength = customtkinter.CTkLabel(exloop_calc_result, text="Minimum Selected Arm Length (m): ").grid(row=4,  column=0, pady=2)

        explength_design = round(totalexpansionlength_design*1000, 3)
        explength_hydro = round(totalexpansionlength_hydro*1000, 3)
        total_expansion_design = customtkinter.CTkLabel(exloop_calc_result, text=str(explength_design) + " mm").grid(row=0,column=1,pady=2)
        total_expansion_hydro = customtkinter.CTkLabel(exloop_calc_result,text=str(explength_hydro) + " mm").grid(row=1,column=1,pady=2)
        minarm_design = customtkinter.CTkLabel(exloop_calc_result, text=str(minimumarmlength_design) + " m").grid(row=2,column=1,pady=2)
        minarm_hydro = customtkinter.CTkLabel(exloop_calc_result, text=str(minimumarmlength_hydro) + " m").grid(row=3, column=1, pady=2)
        recomm_arm = customtkinter.CTkLabel(exloop_calc_result, text=str(reccommended_arm_length) + " m").grid(row=4, column=1, pady=2)
...
rc_maopinfo_label=customtkinter.CTkLabel(rc_calc_frame, text="*For Casing Pipe, Give Zero Value").grid(row=7, column=3, pady=2)

rc_maot_label=customtkinter.CTkLabel(rc_calc_frame, text="Max. Operating Temperature (deg.C): ").grid(row=8, column=0, pady=2)
rc_maotinfo_label=customtkinter.CTkLabel(rc_calc_frame, text="*For Casing Pipe, Input Ambient Temperature").grid(row=8, column=3, pady=2)

rc_ambtemp_label=customtkinter.CTkLabel(rc_calc_frame, text="Minimum Ambient Temperature (deg.C): ").grid(row=9, column=0, pady=2)

rc_installmethod_label=customtkinter.CTkLabel(rc_calc_frame, text= "Installation Method: ").grid(row=10, column=0, pady=2)

rc_soiltype_label=customtkinter.CTkLabel(rc_calc_frame, text="Soil Type: ").grid(row=11, column=0, pady=2)

rc_soilunit_label=customtkinter.CTkLabel(rc_calc_frame, text="Unit Weight of Soil (kN/m3): ").grid(row=12, column=0, pady=2)

rc_soilreaction_label=customtkinter.CTkLabel(rc_calc_frame, text="Modulus of Soil Reaction (ksi): ").grid(row=13, column=0, pady=2)
rc_soilreactioninfo_label=customtkinter.CTkLabel(rc_calc_frame, text="*Refer to Annex A.1 Table A-1 of API 1102", padx=10).grid(row=13, column=3, pady=2)

rc_resilientsoil_label=customtkinter.CTkLabel(rc_calc_frame, text="Resilient Modulus of Soil (ksi): ").grid(row=14, column=0, pady=2)
rc_resilientsoilinfo_label=customtkinter.CTkLabel(rc_calc_frame, text="*Refer to Annex A.1 Table A-2 of API 1102", padx=10).grid(row=14, column=3, pady=2)

rc_pavement_label=customtkinter.CTkLabel(rc_calc_frame, text="Pavement Type: ").grid(row=15, column=0, pady=2)

rc_contactarea_label=customtkinter.CTkLabel(rc_calc_frame, text="Contact Area of Wheel: ").grid(row=16, column=0, pady=2)
rc_contactareainput_label=customtkinter.CTkLabel(rc_calc_frame, text="Contact Area of Wheel (m2): ")
rc_contactareainfo_label=customtkinter.CTkLabel(rc_calc_frame, text="*See Section 4.7.2.2.1 of API 1102").grid(row=16, column=3, pady=2)

rc_axleload_label=customtkinter.CTkLabel(rc_calc_frame, text="Axle Load: ").grid(row=18, column=0, pady=2)
rc_axleloadinput_label=customtkinter.CTkLabel(rc_calc_frame, text="Axle Load (kN): ")
rc_axleloadinfo_label=customtkinter.CTkLabel(rc_calc_frame, text="*See Section 4.7.2.2.1 of API 1102").grid(row=18, column=3, pady=2)

rc_burial_label=customtkinter.CTkLabel(rc_calc_frame, text="Pipe Burial Depth to Top of Pipe (m): ").grid(row=20, column=0, pady=2)
...
exloop_code_label=customtkinter.CTkLabel(exloop_calc_frame, text="Standard Code: ", padx=10).grid(row=0, column=0, pady=2)
exloop_lc_label=customtkinter.CTkLabel(exloop_calc_frame, text="Location Class: ")
exloop_class_label=customtkinter.CTkLabel(exloop_calc_frame, text="Pipe Material Class: ").grid(row=3, column=0, pady=2)
exloop_nps_label=customtkinter.CTkLabel(exloop_calc_frame, text="Pipe NPS (in): ").grid(row=4, column=0, pady=2)
exloop_thk_label=customtkinter.CTkLabel(exloop_calc_frame, text="Pipe Thickness (mm): ").grid(row=5, column=0, pady=2)

exloop_designpressure_label=customtkinter.CTkLabel(exloop_calc_frame, text="Design Pressure (psi): ").grid(row=6, column=0, pady=2)
exloop_designtemperature_label=customtkinter.CTkLabel(exloop_calc_frame, text="Design Temperature (deg.C): ").grid(row=7, column=0, pady=2)
exloop_ambienttemperature_label=customtkinter.CTkLabel(exloop_calc_frame, text="Min. Ambient Temperature (deg.C): ").grid(row=8, column=0, pady=2)
exloop_hydrotestpressure_label=customtkinter.CTkLabel(exloop_calc_frame, text="Hydrotest Pressure: ").grid(row=9, column=0, pady=2)

exloop_spanlength_label=customtkinter.CTkLabel(exloop_calc_frame, text="Effective Span Length (m): ").grid(row=11, column=0, pady=2)
...
exloop_backbutton=customtkinter.CTkButton(root, corner_radius=5, pady=5, text="Back to Main Menu", fg_color="gray75", command=back)
